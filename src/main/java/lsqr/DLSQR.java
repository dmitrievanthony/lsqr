...
public class DLSQR {
     ...
     */
    public static double[] lsqr(Dataset dataset, int n, double damp, double atol, double btol, double conlim, double iter_lim, boolean calc_var, double[] x0) throws ExecutionException, InterruptedException {
        if (iter_lim < 0)
            iter_lim = 2 * n;
        ...
        if (x0 == null) {
            ...
            x = x0;
            beta = dataset.beta(x, -1.0, 1.0);
        }
        double[] v = new double[n];
        double alfa;
        if (beta > 0) {
            v = dataset.iter(beta, v);
